...
describe('groupAccountsByCategory', () => {
    ...
    it('handles fractional balances precisely (string arithmetic)', () => {
        const a = account({ id: 'a', type: 'brokerage', balance: '0.10' });
        const b = account({ id: 'b', type: 'brokerage', balance: '0.20' });

        const [investment] = groupAccountsByCategory([a, b]);

        expect(investment.total).toBe('0.30');
    });
