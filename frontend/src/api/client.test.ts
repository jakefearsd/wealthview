import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import type { AxiosAdapter, AxiosResponse, InternalAxiosRequestConfig } from 'axios';
import axios from 'axios';

function ok(config: InternalAxiosRequestConfig, body: unknown = {}): AxiosResponse {
    return {
        data: body,
        status: 200,
        statusText: 'OK',
        headers: {},
        config,
    };
}

function unauthorized(config: InternalAxiosRequestConfig): AxiosResponse {
    return {
        data: { message: 'Unauthorized' },
        status: 401,
        statusText: 'Unauthorized',
        headers: {},
        config,
    };
}

/**
 * Import a fresh copy of the client module so the module-level `isRefreshing` /
 * `failedQueue` state doesn't leak between tests.
 */
async function freshClient() {
    vi.resetModules();
    const { default: client } = await import('./client');
    return client;
}

describe('api client — request configuration', () => {
    it('uses /api/v1 as the baseURL', async () => {
        const client = await freshClient();
        expect(client.defaults.baseURL).toBe('/api/v1');
    });

    it('sends credentials so HttpOnly auth cookies travel with each request', async () => {
        const client = await freshClient();
        expect(client.defaults.withCredentials).toBe(true);
    });

    it('relies on axios defaults for CSRF (XSRF-TOKEN cookie -> X-XSRF-TOKEN header)', async () => {
        const client = await freshClient();
        // Axios defaults: xsrfCookieName='XSRF-TOKEN', xsrfHeaderName='X-XSRF-TOKEN'.
        // We deliberately do not override them — Spring's CookieCsrfTokenRepository
        // emits a cookie with the same name, so the double-submit pattern works
        // out of the box.
        expect(client.defaults.xsrfCookieName).toBe('XSRF-TOKEN');
        expect(client.defaults.xsrfHeaderName).toBe('X-XSRF-TOKEN');
    });

    it('does not attach an Authorization header — auth is via cookies only', async () => {
        const client = await freshClient();
        const captured: InternalAxiosRequestConfig[] = [];
        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            captured.push(config);
            return ok(config);
        }) as AxiosAdapter;

        await client.get('/accounts');

        expect(captured[0].headers.Authorization).toBeUndefined();
    });
});

describe('api client — 401 response handling', () => {
    let originalAdapter: AxiosAdapter | undefined;
    let originalLocation: Location;

    beforeEach(() => {
        vi.clearAllMocks();
        originalAdapter = axios.defaults.adapter as AxiosAdapter | undefined;
        originalLocation = window.location;
        // @ts-expect-error overriding readonly for test
        delete window.location;
        // @ts-expect-error test shim
        window.location = { href: '' } as Location;
    });

    afterEach(() => {
        axios.defaults.adapter = originalAdapter;
        // @ts-expect-error restore
        window.location = originalLocation;
    });

    it('refreshes via cookie and replays the original request on 401', async () => {
        const client = await freshClient();

        let callCount = 0;
        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            callCount++;
            if (callCount === 1) {
                return Promise.reject({ response: unauthorized(config), config });
            }
            return ok(config, { ok: true });
        }) as AxiosAdapter;

        let refreshBody: unknown = 'unchanged';
        let refreshWithCredentials = false;
        axios.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            refreshBody = config.data;
            refreshWithCredentials = config.withCredentials === true;
            return ok(config, {});
        }) as AxiosAdapter;

        const response = await client.get('/accounts');

        expect(response.data).toEqual({ ok: true });
        // No body sent — server reads the refresh_token cookie.
        expect(refreshBody == null || refreshBody === '').toBe(true);
        expect(refreshWithCredentials).toBe(true);
        expect(callCount).toBe(2);
    });

    it('redirects to /login when the refresh call itself fails', async () => {
        const client = await freshClient();
        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) =>
            Promise.reject({ response: unauthorized(config), config })) as AxiosAdapter;

        axios.defaults.adapter = (async () =>
            Promise.reject(new Error('refresh down'))) as AxiosAdapter;

        await expect(client.get('/accounts')).rejects.toBeDefined();
        expect(window.location.href).toBe('/login');
    });

    it('does not attempt a second refresh for a retried request that 401s again', async () => {
        const client = await freshClient();

        let refreshCalls = 0;
        axios.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            refreshCalls++;
            return ok(config, {});
        }) as AxiosAdapter;

        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) =>
            Promise.reject({ response: unauthorized(config), config })) as AxiosAdapter;

        await expect(client.get('/accounts')).rejects.toBeDefined();
        expect(refreshCalls).toBe(1);
    });

    it('passes through non-401 errors without triggering refresh', async () => {
        const client = await freshClient();

        let refreshCalls = 0;
        axios.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            refreshCalls++;
            return ok(config);
        }) as AxiosAdapter;

        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) =>
            Promise.reject({
                response: {
                    data: { message: 'server error' },
                    status: 500,
                    statusText: 'Internal',
                    headers: {},
                    config,
                },
                config,
            })) as AxiosAdapter;

        await expect(client.get('/accounts')).rejects.toBeDefined();
        expect(refreshCalls).toBe(0);
    });
});

describe('api client — concurrent 401 queueing', () => {
    let originalAdapter: AxiosAdapter | undefined;
    let originalLocation: Location;

    beforeEach(() => {
        vi.clearAllMocks();
        originalAdapter = axios.defaults.adapter as AxiosAdapter | undefined;
        originalLocation = window.location;
        // @ts-expect-error overriding readonly for test
        delete window.location;
        // @ts-expect-error test shim
        window.location = { href: '' } as Location;
    });

    afterEach(() => {
        axios.defaults.adapter = originalAdapter;
        // @ts-expect-error restore
        window.location = originalLocation;
    });

    it('queues concurrent 401s and replays them once refresh completes', async () => {
        const client = await freshClient();

        let callCount = 0;
        client.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            callCount++;
            if (callCount <= 2) {
                return Promise.reject({ response: unauthorized(config), config });
            }
            return ok(config, { ok: true });
        }) as AxiosAdapter;

        let refreshCalls = 0;
        axios.defaults.adapter = (async (config: InternalAxiosRequestConfig) => {
            refreshCalls++;
            await Promise.resolve();
            return ok(config, {});
        }) as AxiosAdapter;

        const [r1, r2] = await Promise.all([client.get('/a'), client.get('/b')]);

        expect(r1.data).toEqual({ ok: true });
        expect(r2.data).toEqual({ ok: true });
        expect(refreshCalls).toBe(1);
    });
});
