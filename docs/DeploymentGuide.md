...
| `SUPER_ADMIN_PASSWORD` | Initial password for `admin@wealthview.local` | Created by `SuperAdminInitializer` on first start. `ProductionConfigValidator` also rejects the dev demo passwords (`admin123`, `demo123`, `LOCAL_DEV_NOT_A_REAL_PASSWORD_OVERRIDE_VIA_ENV`). Change immediately after first login. |
