...
| 6 | Docker `:latest` tag | Already done | `docker-compose.prod.yml` pins `image: wealthview:${WEALTHVIEW_VERSION:?...}` (fails loud without it); the dev `docker-compose.yml`'s `app` service has no `image:` line (local `build: .` only, never pulled by tag). Both files pin `db` to `postgres:16@sha256:...` by digest |
