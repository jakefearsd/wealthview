...
class MonteCarloSpendingOptimizerTest {
    ...
    void splitWithdrawal_allOrdersAndPreAge595_correctDistribution() {
        ...
        assertThat(tf.taxable()).isEqualTo(100);
        assertThat(tf.traditional()).isEqualTo(50);
        assertThat(tf.roth()).isEqualTo(0);
        ...
        assertThat(trf.taxable()).isEqualTo(0);
        assertThat(trf.traditional()).isEqualTo(150);
        assertThat(trf.roth()).isEqualTo(0);
        ...
        assertThat(rf.taxable()).isEqualTo(0);
        assertThat(rf.traditional()).isEqualTo(0);
        assertThat(rf.roth()).isEqualTo(150);
        ...
        assertThat(pre.taxable()).isEqualTo(100); // capped at available taxable
        assertThat(pre.traditional()).isEqualTo(0);
        assertThat(pre.roth()).isEqualTo(0);
    }
    ...
    void splitWithdrawal_dynamicSequencing_drawsTraditionalUpToCeiling() {
        ...
        assertThat(ds.taxable()).isEqualTo(0);
        assertThat(ds.traditional()).isEqualTo(150);
        assertThat(ds.roth()).isEqualTo(0);
        ...
        assertThat(ds0.taxable()).isEqualTo(100);
        assertThat(ds0.traditional()).isEqualTo(0);
        assertThat(ds0.roth()).isEqualTo(50);
        ...
        assertThat(dsPre.taxable()).isEqualTo(100);
        assertThat(dsPre.traditional()).isEqualTo(0);
        assertThat(dsPre.roth()).isEqualTo(0);
        ...
        assertThat(dsRmd.taxable()).isEqualTo(100);
        assertThat(dsRmd.traditional()).isEqualTo(50);
        assertThat(dsRmd.roth()).isEqualTo(0);
    }

    @Test
    void optimize_contingentSpending_p25LowerThanMedianLowerThanP55() {
        var phases = List.of(new GuardrailPhaseInput("All", 62, null, 1));
        var input = new GuardrailOptimizationInput(
                LocalDate.of(2030, 1, 1), 1968, 90, new BigDecimal("0.03"),
                List.of(new HypotheticalAccountInput(
                ...
                new BigDecimal("30000"), BigDecimal.ZERO,
                new BigDecimal("0.10"), new BigDecimal("0.15"),
                500, new BigDecimal("0.80"), phases, 42L,
                BigDecimal.ZERO, null, 0,
                0, BigDecimal.ZERO, null, null,
                false, null, null, 5, null, null);
        ...
        // Check a mid-retirement year (not the first or last)
        var midYear = result.yearlySpending().get(result.yearlySpending().size() / 2);

        assertThat(midYear.contingentSpendingP25()).isNotNull();
        assertThat(midYear.contingentSpendingMedian()).isNotNull();
        assertThat(midYear.contingentSpendingP55()).isNotNull();

        // P25 portfolio is smaller -> less sustainable spending
        assertThat(midYear.contingentSpendingP25().doubleValue())
                .isLessThanOrEqualTo(midYear.contingentSpendingMedian().doubleValue());
        assertThat(midYear.contingentSpendingMedian().doubleValue())
                .isLessThanOrEqualTo(midYear.contingentSpendingP55().doubleValue());
    }

    @Test
    void optimize_contingentSpending_allYearsPopulated() {
        var phases = List.of(new GuardrailPhaseInput("All", 62, null, 1));
        var input = buildInput(
                new BigDecimal("1000000"),
                new BigDecimal("30000"),
        ...
        for (var year : result.yearlySpending()) {
            assertThat(year.contingentSpendingP25())
                    .as("contingentSpendingP25 should be non-null for age %d", year.age())
                    .isNotNull();
            assertThat(year.contingentSpendingMedian())
                    .as("contingentSpendingMedian should be non-null for age %d", year.age())
                    .isNotNull();
            assertThat(year.contingentSpendingP55())
                    .as("contingentSpendingP55 should be non-null for age %d", year.age())
                    .isNotNull();
            assertThat(year.contingentSpendingP25().doubleValue())
                    .as("contingentSpendingP25 should be non-negative for age %d", year.age())
                    .isGreaterThanOrEqualTo(0);
        }
    }

    @Test
    void optimize_contingentSpending_lastYearApproachesZero() {
        var phases = List.of(new GuardrailPhaseInput("All", 62, null, 1));
        var input = buildInput(
                new BigDecimal("500000"),
                new BigDecimal("20000"),
        ...
        var result = optimizer.optimize(input);
        var lastYear = result.yearlySpending().get(result.yearlySpending().size() - 1);

        // In the last year there's essentially no remaining years to sustain spending,
        // so contingent spending should be very small (close to zero or the balance itself)
        assertThat(lastYear.contingentSpendingP25()).isNotNull();
